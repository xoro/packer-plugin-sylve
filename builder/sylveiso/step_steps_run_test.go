...
func TestStepStartVM_RunUntilRunning(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasPrefix(r.URL.Path, "/api/vm/9") && r.Method == http.MethodGet && strings.Contains(r.URL.RawQuery, "type=rid"):
			vm := client.VM{RID: 9, State: client.DomainStateRunning, Networks: []client.VMNetwork{}}
...
func TestStepStartVM_AcceptsNoStateAsRunning(t *testing.T) {
	restoreStartVMDurations(t)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch {
		case r.URL.Path == "/api/vm/start/9" && r.Method == http.MethodPost:
			_ = json.NewEncoder(w).Encode(client.APIResponse[interface{}]{Status: "ok"})
		case strings.HasPrefix(r.URL.Path, "/api/vm/9") && r.Method == http.MethodGet && strings.Contains(r.URL.RawQuery, "type=rid"):
			vm := client.VM{RID: 9, State: client.DomainStateNoState, Networks: []client.VMNetwork{}}
			_ = json.NewEncoder(w).Encode(client.APIResponse[client.VM]{Status: "ok", Data: vm})
		case r.URL.Path == "/api/vm/logs/9" && r.Method == http.MethodGet:
			_ = json.NewEncoder(w).Encode(client.APIResponse[struct {
				Logs string `json:"logs"`
...
func TestStepStartVM_TimesOutWaitingForRunning(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/api/vm/start/9" && r.Method == http.MethodPost:
			_ = json.NewEncoder(w).Encode(client.APIResponse[interface{}]{Status: "ok"})
		case strings.HasPrefix(r.URL.Path, "/api/vm/9") && r.Method == http.MethodGet && strings.Contains(r.URL.RawQuery, "type=rid"):
			vm := client.VM{RID: 9, State: client.DomainStateBlocked, Networks: []client.VMNetwork{}}
			_ = json.NewEncoder(w).Encode(client.APIResponse[client.VM]{Status: "ok", Data: vm})
		default:
			http.NotFound(w, r)
		}
...
func TestStepStartVM_ContextCancelledDuringPoll(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == fmt.Sprintf("/api/vm/start/%d", rid) && r.Method == http.MethodPost:
			_ = json.NewEncoder(w).Encode(client.APIResponse[interface{}]{Status: "ok"})
		case strings.HasPrefix(r.URL.Path, fmt.Sprintf("/api/vm/%d", rid)) && r.Method == http.MethodGet && strings.Contains(r.URL.RawQuery, "type=rid"):
			vm := client.VM{RID: rid, State: client.DomainStateShutoff}
			_ = json.NewEncoder(w).Encode(client.APIResponse[client.VM]{Status: "ok", Data: vm})
		default:
			http.NotFound(w, r)
		}
