...
func TestArtifact_BuilderId(t *testing.T) {
	a := &Artifact{VMRID: 1, VMID: 2}
	if a.BuilderId() != BuilderID {
		t.Errorf("BuilderId() = %q, want %q", a.BuilderId(), BuilderID)
	}
}
...
func TestArtifact_String(t *testing.T) {
	a := &Artifact{VMRID: 7, VMID: 3}
	want := "Sylve VM RID 7 (ID 3)"
	if a.String() != want {
...
func TestArtifact_State(t *testing.T) {
	a := &Artifact{StateData: map[string]interface{}{"vm_rid": uint(5)}}
	if v, ok := a.State("vm_rid").(uint); !ok || v != 5 {
		t.Errorf("State(\"vm_rid\") = %v, want uint(5)", a.State("vm_rid"))
	}
	if a.State("missing") != nil {
		t.Error("State for missing key should return nil")
	}
}
