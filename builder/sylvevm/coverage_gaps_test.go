...
func TestStepDiscoverIP_PollErrorContinues(t *testing.T) {
	...
	discoverIPTotalTimeout = 80 * time.Millisecond
...
func TestStepDiscoverIP_ContextCancel(t *testing.T) {
	origPoll := discoverIPPollInterval
	discoverIPPollInterval = 50 * time.Millisecond
	t.Cleanup(func() { discoverIPPollInterval = origPoll })
...
func TestStepDeleteVM_DestroyFalse_Skip(t *testing.T) {
	...
	if (&StepDeleteVM{Config: cfg}).Run(context.Background(), state) != multistep.ActionContinue {
