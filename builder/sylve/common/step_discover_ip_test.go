...
func TestStepDiscoverIP_Run_ViaDHCPLease(t *testing.T) {
	...
	step := &StepDiscoverIP{SylveURL: srv.URL, SylveToken: "tok", TLSSkipVerify: true}
...
// TestStepDiscoverIP_DHCPPollErrorThenLease covers the error-then-success path.
...
func TestStepDiscoverIP_NoMAC(t *testing.T) {
	step := &StepDiscoverIP{SylveURL: "http://localhost", SylveToken: "tok"}
	state := newTestState(t)
	// vm_mac not set

	if got := step.Run(context.Background(), state); got != multistep.ActionHalt {
		t.Fatalf("Run() = %v, want ActionHalt when vm_mac missing", got)
	}
}
...
func TestStepDiscoverIP_ContextCancelled(t *testing.T) {
	...
	if got := step.Run(ctx, state); got != multistep.ActionHalt {
		t.Fatalf("Run() = %v, want ActionHalt on cancelled ctx", got)
	}
}
