...
func restoreDiscoverIPDurations(t *testing.T) {
	t.Helper()
	origPoll, origMax := sylvecommon.DiscoverIPPollInterval, sylvecommon.DiscoverIPTotalTimeout
	t.Cleanup(func() {
		sylvecommon.DiscoverIPPollInterval = origPoll
		sylvecommon.DiscoverIPTotalTimeout = origMax
	})
	sylvecommon.DiscoverIPPollInterval = 1 * time.Millisecond
	sylvecommon.DiscoverIPTotalTimeout = 5 * time.Second
}

func TestStepDeleteVM_DestroyFalse(t *testing.T) {
	step := &sylvecommon.StepDeleteVM{Destroy: false}
...
func TestStepDeleteVM_DestroyTrue_NoRID(t *testing.T) {
	step := &sylvecommon.StepDeleteVM{SylveURL: "http://127.0.0.1:9", SylveToken: "t", TLSSkipVerify: true, Destroy: true}
	state := new(multistep.BasicStateBag)
	state.Put("ui", newMockUI())
	if got := step.Run(context.Background(), state); got != multistep.ActionContinue {
		t.Fatalf("got %v", got)
...
func TestStepDeleteVM_DestroyTrue_Deletes(t *testing.T) {
	...
	step := &sylvecommon.StepDeleteVM{SylveURL: srv.URL, SylveToken: "tok", TLSSkipVerify: true, Destroy: true}
	state := new(multistep.BasicStateBag)
	state.Put("ui", newMockUI())
...
func TestStepDiscoverIP_ContextCancelled(t *testing.T) {
	...
	step := &sylvecommon.StepDiscoverIP{SylveURL: "http://127.0.0.1:9", SylveToken: "t", TLSSkipVerify: true}
	state := new(multistep.BasicStateBag)
	state.Put("ui", newMockUI())
...
func TestStepDiscoverIP_DHCPLease(t *testing.T) {
	...
	step := &sylvecommon.StepDiscoverIP{SylveURL: srv.URL, SylveToken: "tok", TLSSkipVerify: true}
	state := new(multistep.BasicStateBag)
	state.Put("ui", newMockUI())
	state.Put("vm_mac", "aa:bb:cc:dd:ee:ff")
